...
type ProfitLossReportSvc interface {
	GetProfitLoss(ctx context.Context, typ string, month, year int, store string) (*service.ProfitLoss, error)
}
...
func (h *ProfitLossReportHandler) handleGet(c *gin.Context) {
	...
	res, err := h.svc.GetProfitLoss(context.Background(), typ, month, year, store)
	if err != nil {
